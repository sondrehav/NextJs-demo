import {
  ContentTransformer as CContentTransformer,
  NodeContent,
  NodeProps,
  Overrides,
  Props as CProps,
...
const LinkContent = (props: NodeProps) => {
  ...
  if ((props.metadata.href as string).startsWith("/")) {
    return (
      <Link href={props.metadata.href}>
        <a
          className={
            "border-b-2 border-gray-300 transition-all hover:border-gray-100"
          }
        >
          <NodeContent {...props} />
        </a>
...
const QuoteContent = (props: NodeProps) => (
...
const ParagraphContent = (props: NodeProps) => (
  <p className={"my-4"}>
    <NodeContent {...props} />
  </p>
);
...
const CodeContent = (props: NodeProps) => {
  ...
  const language = children[0].textContent
    ? r.exec(children[0].textContent)?.[1]
    : undefined;
...
const UListContent = (props: NodeProps) => (
  <ul className={"list-disc"}>
    {(props.children ?? [])
      .filter((c) => c.type === "list-item")
      .map((c, i) => (
        <li key={i} className={"list-item ml-6"}>
          <NodeContent {...c} />
        </li>
      ))}
  </ul>
);
const OListContent = (props: NodeProps) => (
