...
export type LinkItemProps = { label: string; path: string };
      ...
      <div className={classNames(container, "relative px-0 z-50")}>
        <div className="relative flex items-center justify-between h-16">
          ...
          <div className={"flex flex-row items-center space-x-1"}>
            <a
              className={"flex-grow-0 flex-shrink-0 p-2"}
              href={"https://github.com/sondrehav/NextJs-demo"}
            >
              <GithubSquare
                className={
                  "fill-current transform transition-all scale-90 hover:scale-100 cursor-pointer"
                }
                width={32}
                height={32}
              />
            ...
            <Link href={`/edit?path=${currentPath}`} prefetch={false}>
              <a className={"flex-grow-0 flex-shrink-0 p-2"}>
                <Pencil
                  className={
                    "fill-current transform transition-all scale-90 hover:scale-100 cursor-pointer"
                  }
        ...
        <div className="px-2 pt-2 pb-3 space-y-1">
          {links.map((link) => (
            <Fragment key={link.path}>
              {link.path === `/web${currentPath === "/" ? "" : currentPath}` ? (
                <span
                ...
                <Link href={link.path!.replace("/web", "")}>
                  <a
                    className={
